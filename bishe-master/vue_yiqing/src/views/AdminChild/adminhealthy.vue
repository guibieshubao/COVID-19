        ...
        <el-form ref="form" :model="qeryinfo" label-width="80px" inline>
            ...
            <el-form-item label="学院">
                <el-select v-model="qeryinfo.college" clearable placeholder="请选择">
                  <el-option label="计网学院" value="计网学院"> </el-option>
...
export default {
  components: { uploadHealthy, DownloadHealthy },
    ...
    methods: {
       // 1.上传文件之前的钩子, 参数为上传的文件,若返回 false 或者返回 Promise 且被 reject，则停止上传
  ...
  　　　　　　const tHeader = ['索引','学号','姓名','体温','是否发烧','是否经过高危地区','是否核酸','填报时间','学院'];
  　　　　　　const filterVal = ['id', 'u_id', 'username', 'temperature', 'hot', 'gohighrisk','ishesuan','filldata','college' ];
  　　　　　　const list = res.list;
  　　　　　　const data = this.formatJson(filterVal, list);
  　　　　　　export_json_to_excel(tHeader, data, '健康填报管理列表');
  　　　　})
  　 },
